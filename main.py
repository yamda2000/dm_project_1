...
if not "initialized" in st.session_state:
    # Streamlit が提供している セッションごとの状態（state）を保持するための辞書型オブジェクト
...
# 6. チャット入力の受け付け
############################################################
chat_message = st.chat_input(ct.CHAT_INPUT_HELPER_TEXT) # 顔マークの右にチェットメッセージが表示される
...
if chat_message:
    ...
    with st.spinner(ct.SPINNER_TEXT): # SPINNER_TEXT = "回答生成中..."
        try:
